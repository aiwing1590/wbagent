...
if file:
    ...
    with tab_ai:
        ...
        if query:
            ...
            # --- ПРЕДОХРАНИТЕЛЬ: ЕСЛИ ИИ И ЛОКАЛЬНЫЙ ПОИСК ДАЛИ ПУСТОЙ РЕЗУЛЬТАТ ---
            if not resp_text and not fallback_text:
                fallback_text = (
                    "⚠️ **Ответ не получен**\n\n"
                    "**Что можно сделать:**\n"
                    "* **Если ты ещё не настроил API-ключ Gemini:** В данный момент работает только локальный анализ. Попробуй нажать на одну из трех кнопок быстрого выбора выше (например, *«Топ-3 товара по прибыли»*) — они рассчитают всё мгновенно по формулам!\n"
                    "* **Если ты ищешь конкретный товар:** Убедись, что вводишь его название точно так же, как написано в твоей Excel-таблице (проверь регистр букв и пробелы).\n"
                    "* **Если ключ Gemini уже добавлен в Secrets:** Возможно, в самом ключе опечатка, либо на твоем аккаунте Google AI Studio закончились бесплатные лимиты запросов."
                )
