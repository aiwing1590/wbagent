...
def load_spp_orders(
    ...
    with psycopg.connect(DATABASE_URL, row_factory=dict_row) as connection:
        with connection.cursor() as cursor:
            filters = ["so.owner_login = %s", "so.order_datetime >= %s", "so.order_datetime < %s"]
            parameters = [
                owner_login,
                start.to_pydatetime(),
            ...
            optional_filters = [
                (brand, "LOWER(COALESCE(tp.brand, so.brand, '')) = LOWER(%s)"),
                (subject, "LOWER(COALESCE(tp.subject, so.subject, '')) = LOWER(%s)"),
                (wb_article, "so.wb_article = %s"),
                (warehouse, "LOWER(so.warehouse) = LOWER(%s)"),
                (region, "LOWER(so.region) = LOWER(%s)"),
            ]
            for value, expression in optional_filters:
                if str(value or "").strip():
                    filters.append(expression)
                    parameters.append(int(value) if expression == "so.wb_article = %s" else str(value).strip())
    ...
    present_days = set(pd.to_datetime(frame["Дата заказа"]).dt.date)
    expected_days = list(pd.date_range(start.date(), end.date(), freq="D").date)
    ...
    stats = {
        ...
        "coverage": {
            ...
            "requested_to": end.date().isoformat(),
            "days_with_data": len(present_days),
...
def spp_period_snapshot(owner_login: str, date_from: str, date_to: str, **filters) -> dict:
    frame, match_stats = load_spp_orders(owner_login, date_from, date_to, **filters)
    analysis = build_spp_analysis(frame, match_stats, None, None)
    return analysis


def metric_delta(current, previous) -> dict:
    ...
    return {
        ...
        "change_percent": (
            round((current_value / previous_value - 1) * 100, 2)
            if previous_value else None
        ),
    }


def compare_spp_periods(owner_login: str, first_from: str, first_to: str,
                        second_from: str, second_to: str, **filters) -> dict:
    first = spp_period_snapshot(owner_login, first_from, first_to, **filters)
    second = spp_period_snapshot(owner_login, second_from, second_to, **filters)
    metrics = {
    ...
    def indexed(rows: list[dict], key: str) -> dict:
        return {str(row.get(key) or "Не указано"): row for row in rows}

    def compare_rows(current_rows: list[dict], previous_rows: list[dict], key: str) -> list[dict]:
        current_index = indexed(current_rows, key)
        previous_index = indexed(previous_rows, key)
        result = []
        for name in set(current_index) | set(previous_index):
...
async def analyze_saved_spp_period(
    ...
    owner_login = get_current_user(session_token)
    stored_df, match_stats = load_spp_orders(
        owner_login, date_from, date_to, brand, subject,
        wb_article, warehouse, region,
    )
...
@app.post("/spp/compare")
async def spp_compare(
    session_token: str = Form(...),
    first_from: str = Form(...),
    first_to: str = Form(...),
    second_from: str = Form(...),
    second_to: str = Form(...),
    brand: str = Form(""),
    subject: str = Form(""),
    wb_article: str = Form(""),
    ...
    owner_login = get_current_user(session_token)
    return compare_spp_periods(
        owner_login, first_from, first_to, second_from, second_to,
        brand=brand, subject=subject, wb_article=wb_article,
        warehouse=warehouse, region=region,
    )


@app.post("/spp/product-card")
async def spp_product_card(
    session_token: str = Form(...),
    wb_article: str = Form(...),
    date_from: str = Form(...),
    date_to: str = Form(...),
    ...
    owner_login = get_current_user(session_token)
    analysis = spp_period_snapshot(
        owner_login, date_from, date_to, wb_article=wb_article,
    )
    product = (analysis.get("products") or [{}])[0]
    return {
        "wb_article": wb_article,
        "name": product.get("product") or f"Артикул {wb_article}",
        "period": analysis["period"],
        "summary": analysis["summary"],
        "daily": analysis["daily"],
        "regions": spp_group_dynamics(
            load_spp_orders(owner_login, date_from, date_to, wb_article=wb_article)[0]
            .rename(columns={
                "Дата заказа": "datetime", "СПП": "spp", "Регион": "region",
                "Склад": "warehouse", "Цена со скидкой": "our_price",
                "Цена продажи": "sale_price",
            }).assign(day=lambda value: pd.to_datetime(value["datetime"]).dt.date),
            ["region"], limit=50, minimum_orders=1,
        ),
        "warehouses": spp_group_dynamics(
            load_spp_orders(owner_login, date_from, date_to, wb_article=wb_article)[0]
            .rename(columns={
                "Дата заказа": "datetime", "СПП": "spp", "Регион": "region",
                "Склад": "warehouse", "Цена со скидкой": "our_price",
            ...
            ["warehouse"], limit=50, minimum_orders=1,
        ),
    }
